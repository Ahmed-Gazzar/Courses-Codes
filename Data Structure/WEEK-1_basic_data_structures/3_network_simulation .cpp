...
public:
    ...
    Response Process(const Request& request) {
        ...
        if (finish_time_.empty()) {
            ...
            finish_time_2.push(request.arrival_time + request.process_time);
            ...
            if (finish_time_.size() == size_) {
                if (request.arrival_time >= finish_time_.front()) {
                    finish_time_.pop();
                    startTime = max(request.arrival_time, finish_time_2.top());
                    finish_time_.push(startTime + request.process_time);
                    finish_time_2.push(startTime + request.process_time);
                }
                else {
                    dropped = true;
                }
